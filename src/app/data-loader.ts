...
export class DataLoaderManager implements AppModule {
  ...
  async loadAllData(): Promise<void> {
    ...
    if (SITE_VARIANT !== 'happy' && (this.ctx.mapLayers.flights || this.ctx.mapLayers.militaryAircraftUnknown)) tasks.push({ name: 'flights', task: runGuarded('flights', () => this.loadFlightDelays()) });
  ...
  async loadIntelligenceSignals(): Promise<void> {
    ...
    tasks.push((async () => {
      try {
        ...
        if (
          this.ctx.mapLayers.military
          || this.ctx.mapLayers.militaryAircraftConfirmed
          || this.ctx.mapLayers.militaryAircraftUnknown
          || this.ctx.mapLayers.navalActivity
        ) {
