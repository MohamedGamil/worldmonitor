...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // Set VITE_BACKEND_URL in .env.local to override (default: http://localhost:4000).
      // In production, VITE_WS_API_URL causes installWebApiRedirect() in runtime.ts
      // to redirect /api/* fetches to the backend at the browser level.
      '/api': {
        target: process.env.VITE_BACKEND_URL || 'http://localhost:4000',
        changeOrigin: true,
        configure: (proxy) => {
          proxy.on('proxyReq', (proxyReq) => {
            // Same-origin browser requests don't include an Origin header (CORS spec).
            // Inject it so the backend's validateApiKey() recognises this as a
            // trusted localhost dev origin and skips the API key requirement.
